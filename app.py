...
# ---------------------------------------------------------------------
# STREAMLIT SIMPLE PASSWORD PROTECTION
# ---------------------------------------------------------------------
...
def check_password():
    ...
    def password_entered():
        ...
        # Safety Check to prevent pipeline-click re-run crashes
    ...
    st.set_page_config(
        page_title="Lock - Fleet Health Intelligence", page_icon="🔒", layout="centered")
    st.markdown("<h2 style='text-align: center;'>🔒 Fleet Intelligence Access Control</h2>",
                unsafe_allow_html=True)
    st.write("---")

    with st.container():
        st.text_input(
            "Enter Security Authorization Password:",
            type="password",
        ...
        if "password_correct" in st.session_state and not st.session_state["password_correct"]:
            st.error("❌ Access Denied: Invalid Security Password.")
...
if check_password():
    ...
    # ---------------------------------------------------------------------
    # DATA MANAGEMENT & FILE UPLOADS (SIDEBAR)
    # ---------------------------------------------------------------------
    st.sidebar.header("🔄 Data Integration Engine")
    st.sidebar.caption("Update base matrix spreadsheets below:")

    # File upload widgets for both tracking spreadsheets
    uploaded_infotec = st.sidebar.file_uploader(
        "Upload 'infotec.xlsx'", type=["xlsx"])
    uploaded_consumibles = st.sidebar.file_uploader(
        "Upload 'consumibles.xlsx'", type=["xlsx"])

    # Pipeline Trigger Execution Interface Button
    if st.sidebar.button("Import Data & Re-run Pipeline"):
        with st.spinner("Processing files & recalculating lifecycles... Please wait."):
            try:
    ...
    # Load the optimized procurement ledger file
    df = load_data()
    if not df.empty:
        ...
        # ---------------------------------------------------------------------
        # DATA RECONCILIATION & TEMPORAL SPLIT
        # ---------------------------------------------------------------------
        df = df.rename(columns={"Age (Days)": "Calculated Age (Days)"})
        ...
        df['Days to 50% Health'] = (
            df['parsed_projected_date'] - base_date).dt.days
    ...
    /* Primary execution layout styling targeting the sidebar button override */
    ...
    /* Style corrections for File Upload zones inside the dark sidebar */
    ...
    # ---------------------------------------------------------------------
    # PRESENTATION RUN LAYER
    # ---------------------------------------------------------------------
    if df.empty:
        st.info("💡 Complete dashboard visualizations will emerge once the data infrastructure ledger files are successfully built using the control panel on the left.")
    else:
        # Header Title Card
        st.markdown("""
        <div class="hero">
        ...
        # ---------------------------------------------------------------------
        # SIDEBAR CONTROL FILTERS (ONLY VISIBLE IF DATA EXISTS)
        # ---------------------------------------------------------------------
        st.sidebar.markdown("---")
        ...
        filtered_op_df = op_base[
            ...
            (op_base["Current Health (%)"].between(
        ...
        cards = [
            ...
            ("Stable Units", normal_units,
             "Parts running without constraints", "#2E7D32"),
        ...
        # ---------------------------------------------------------------------
        st.markdown(
            '<div class="section-title">Executive Overview</div>', unsafe_allow_html=True)
        ...
        with summary_col2:
            buy_30 = len(
                filtered_op_df[filtered_op_df["Buy in 30 Days?"] == "Yes"])
            st.markdown(f"""
            <div class="info-card">
        ...
        backlog_cols = ["Truck ID", "Installation Date", "Component Description",
                        "Calculated Age (Days)", "Current Health (%)", "Projected 50% Date", "Days to 50% Health"]
        backlog_cols = [
            c for c in backlog_cols if c in filtered_backlog_df.columns]
        ...
        if total_backlog_count > 0:
            ...
            st.download_button(
                ...
                file_name="fleet_maintenance_backlog.csv", mime="text/csv", key="backlog_dl"
            )
        else:
            st.success(
        ...
        # ---------------------------------------------------------------------
        st.markdown(
            '<div class="section-title">Active Part Units Needed by Truck Unit</div>', unsafe_allow_html=True)
        if len(filtered_op_df) > 0:
            truck_distribution = (
                filtered_op_df.groupby("Truck ID").size().reset_index(
                    name="Physical Units Required")
                .sort_values("Physical Units Required", ascending=False).head(25)
            )
        ...
        # ---------------------------------------------------------------------
        st.markdown(
            '<div class="section-title">Priority Operational Procurement Watchlist</div>', unsafe_allow_html=True)
        ...
        # ---------------------------------------------------------------------
        st.markdown(
            '<div class="section-title">Forward Procurement Planning Horizon</div>', unsafe_allow_html=True)

        planning_cols = ["Truck ID", "Installation Date", "Component Description",
                         "Calculated Age (Days)", "Current Health (%)", "Projected 50% Date", "Purchase Status", "Buy in 30 Days?"]
        planning_cols = [
            c for c in planning_cols if c in filtered_op_df.columns]
        ...
        if lookup_truck:
            ...
            if len(truck_df) > 0:
                ...
                truck_urgent = len(
                    truck_df[truck_df["Purchase Status"] == "URGENT"])

                tcol1, tcol2, tcol3 = st.columns(3)
                tcol1.metric("Total Spare Parts Tracking",
                             f"{len(truck_df):,} Units")
                tcol2.metric("Urgent Changes Pending",
                             f"{truck_urgent:,} Units")
                tcol3.metric("Mean Part Health", f"{truck_avg_health:.1f}%")

                truck_cols = ["Installation Date", "Component Description",
                              "Current Health (%)", "Projected 50% Date", "Purchase Status", "Buy in 30 Days?"]
        ...
        # ---------------------------------------------------------------------
        st.markdown(
            '<div class="section-title">Operational Component Inventory</div>', unsafe_allow_html=True)

        display_cols = ["Truck ID", "Installation Date", "Component Description",
                        "Calculated Age (Days)", "Current Health (%)", "Projected 50% Date", "Purchase Status", "Buy in 30 Days?"]
        ...
        with st.expander("View full filtered operational unit inventory table", expanded=False):
            st.dataframe(
                filtered_op_df[display_cols].style.apply(
                    highlight_columns, axis=1),
        ...
        st.download_button(
            ...
            file_name="maintenance_predictions_filtered.csv", mime="text/csv"
        )
